...
Before substantial visual changes with an unclear source, inspect the supplied reference and existing design context; use an available Product Design workflow when useful. Do not assume a missing skill is callable. When the user gives durable prototype-specific design feedback, preferences, or decisions, record them in `AGENTS.md`.
